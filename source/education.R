...
response <- 
  ...
  recode_to(to = 6, where = tertiary_degree == "none" & vocational_degree == "none" & 
  ...
  # recode_to(to = XXXX, where.selected.exactly = "dnk") %>%
  ...
  recode_to(to = 4, where = time_school == "31_60" & transport_school %in% c("walking", "bicycle", "cart")) %>%
  recode_to(to = 5, where.selected.exactly = "above180", source = time_school) %>%
  recode_to(to = 5, where = time_school == "60_180" & transport_school %in% c("walking", "bicycle", "cart")) %>%
  recode_to(to = 6, where = time_school == "above180" & transport_school %in% c("walking", "bicycle", "cart")) %>%
