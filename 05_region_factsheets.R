# Loading in data set, analysis plan, and questionnaire
library(dplyr)
library(hypegrammaR)
...
results <- from_analysisplan_map_to_output(df, 
...
big_big_table <- results$results %>% lapply(function(x) x[["summary.statistic"]]) %>% do.call(rbind, .) %>%
...
data_merge_region %>% write.csv("output/data_merge/large_table_part1_region.csv", row.names = F)
